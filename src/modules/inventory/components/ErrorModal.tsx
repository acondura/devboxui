      ...
      <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-3xl shadow-2xl overflow-hidden transform transition-all animate-in fade-in zoom-in duration-200 text-left">
        <div className="p-6 space-y-6">
          <div className="flex items-start space-x-4">
            <div className="flex-shrink-0 w-12 h-12 bg-rose-100 dark:bg-rose-900/30 rounded-2xl flex items-center justify-center text-rose-600 dark:text-rose-400">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <div className="space-y-1.5 flex-1">
              <h3 className="text-xl font-bold text-slate-900 dark:text-zinc-100">{title}</h3>
              <div className="text-xs text-slate-650 dark:text-zinc-300 font-mono bg-slate-50 dark:bg-zinc-800 p-3 rounded-lg border border-slate-100 dark:border-zinc-700 overflow-x-auto max-h-40 whitespace-pre-wrap leading-relaxed">
                {message}
              </div>
              {friendlyExplanation && (
                <div className="text-xs font-semibold text-rose-700 dark:text-rose-300 mt-2 bg-rose-50 dark:bg-rose-900/20 border border-rose-200/50 dark:border-rose-800/50 p-2.5 rounded-lg leading-normal">
                  💡 {friendlyExplanation}
                </div>
              )}
            </div>
