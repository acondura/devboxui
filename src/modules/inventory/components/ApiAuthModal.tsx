      ...
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden transform transition-all animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-3">
            ...
            {peerCandidates.length === 0 ? (
              <div className="text-center py-6 px-4 bg-slate-950/40 border border-slate-800/80 rounded-xl text-slate-500 text-xs">
                ...
                {peerCandidates.map(peer => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-3.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[10px] text-slate-500 font-mono mt-0.5 flex items-center gap-1.5">
                            ...
                            {isOffline && (
                              <span className="text-rose-500 font-bold uppercase text-[9px] tracking-tight">
                                (Offline)
                              </span>
                            )}
                          </div>
                      ...
                      <div className="text-[10px] bg-slate-800 border border-slate-700 px-2 py-0.5 rounded uppercase font-bold text-slate-400 tracking-wider">
        ...
        <div className="px-6 py-4 bg-slate-950/40 border-t border-slate-800 flex justify-end gap-3">
          <button
            onClick={onClose}
            disabled={isSaving}
            className="px-4 py-2 text-xs font-bold uppercase bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg transition-all"
          >
            Cancel
          </button>
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="px-4 py-2 text-xs font-bold uppercase bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-600 text-white rounded-lg transition-all flex items-center justify-center gap-2"
          >
            {isSaving ? (
              <>
