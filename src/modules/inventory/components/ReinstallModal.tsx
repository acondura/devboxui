    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl shadow-2xl overflow-hidden transform transition-all animate-in fade-in zoom-in duration-200 text-left">
        ...
        <div className="p-6 space-y-6">
          {isAutomated ? (
            <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-xl flex items-start space-x-3">
              ...
              <div className="space-y-1">
                <p className="text-sm font-bold text-amber-900 dark:text-amber-300 uppercase tracking-tight">Destructive Action</p>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-zinc-700 rounded-xl p-4 space-y-3">
                <p className="text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest">What this script does:</p>
                <ul className="grid grid-cols-2 gap-x-4 gap-y-2">
                  {[
                    ...
                    <li key={i} className="flex items-center space-x-2 text-sm text-slate-650 dark:text-zinc-300">
          ...
          <div className="space-y-3">
            {!isAutomated && (
              <div className="space-y-2 pt-2">
                <label className="text-xs font-bold text-slate-600 dark:text-zinc-400 uppercase tracking-widest">Command</label>
                {isLoading ? (
                  <div className="w-full bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-lg p-8 flex flex-col items-center justify-center space-y-3">
                    <div className="h-5 w-5 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin" />
                    <span className="text-[10px] font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest animate-pulse">Regenerating Script...</span>
                  </div>
                ) : bootstrapCommand ? (
                  <>
                    <div className="relative group">
                      <div className="w-full bg-slate-100 dark:bg-zinc-800 border border-slate-250 dark:border-zinc-700 rounded-lg p-4 font-mono text-sm text-slate-800 dark:text-zinc-200 break-all leading-relaxed pr-10 max-h-[120px] overflow-y-auto scrollbar-thin">
                        {bootstrapCommand}
                      </div>
                      <button
                        onClick={handleCopy}
                        className="absolute top-3 right-3 p-1.5 bg-slate-200 dark:bg-zinc-700 hover:bg-slate-300 dark:hover:bg-zinc-600 text-slate-650 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-zinc-100 rounded-md opacity-0 group-hover:opacity-100 transition-all"
                        title="Copy command"
                      >
                        {copyStatus ? (
                    ...
                    <p className="text-sm text-slate-500 dark:text-zinc-400 italic mt-3 leading-relaxed">
          ...
          <div className="flex space-x-3 pt-2">
            <button
              onClick={onClose}
              className="flex-1 px-4 py-3 bg-slate-100 dark:bg-zinc-700 hover:bg-slate-200 dark:hover:bg-zinc-600 text-slate-700 dark:text-zinc-200 font-bold rounded-xl transition-all"
            >
              Close
            </button>
            {isAutomated && (
